import { Intersection, Material, Mesh } from 'three';
import { IfcMesh } from 'web-ifc-three/IFC/BaseDefinitions';
import { IFCLoader } from 'web-ifc-three/IFCLoader';
import { IfcComponent, Context } from '../../base-types';
export declare class IfcSelection extends IfcComponent {
    private context;
    mesh: Mesh | null;
    private selected;
    private modelID;
    private readonly material;
    private loader;
    private readonly scene;
    constructor(context: Context, loader: IFCLoader, material?: Material);
    pick: (item: Intersection, focusSelection?: boolean, duration?: number | undefined) => Promise<{
        modelID: number;
        id: number;
    } | null>;
    unpick(): void;
    pickByID: (modelID: number, ids: number[], focusSelection?: boolean, duration?: number | undefined) => void;
    newSelection: (ids: number[]) => void;
    removeSelectionOfOtherModel(mesh?: IfcMesh): void;
    private focusSelection;
}
